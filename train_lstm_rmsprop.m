...
% the RMSprop optimization method. It utilizes a moving average of 
% squared gradients to normalize the gradient updates, providing 
% robustness against vanishing or exploding gradients.

clear; clc;

% --- 1. DATA LOADING AND INITIALIZATION ---
disp('Caricamento dati e inizializzazione RMSprop...');
...
fprintf('Inizio Training RMSprop: %d Epoche\n', EPOCHS);
...
for epoch = 1:EPOCHS
    ...
    if epoch == 1 || val_acc > max(val_acc_history(1:end-1))
        save('best_lstm_rmsprop.mat', 'lstm_net', 'loss_history', 'val_acc_history', 'VOCAB_SIZE', 'SEQ_LENGTH');
    end
end

% visualization
figure;
subplot(2,1,1); plot(loss_history, 'b-o'); title('Training Loss RMSprop');
